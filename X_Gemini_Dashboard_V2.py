...
st.title("CodeSage - Software Debugging Platform")
...
if col1.button("🔄 Run Pipeline and Load Logs"):
    ...
    if start_date > end_date:
        ...
        notebook_path = os.getenv("NOTEBOOK_PATH")
        print('notebook path inside dashboard', notebook_path)
        print('data existence check:', os.getcwd())


        with st.spinner("Running log pipeline..."):
            print('Data check :', notebook_path)
            result = subprocess.run([
                "python3",
                notebook_path + "X_Gemini_pipeline_V2.py",
...
st.success(f"Latest saved file{data_path + input_folder + f'{latest_file_name}'}")
df.to_csv( data_path + '6_RCA/' + latest_file_name)